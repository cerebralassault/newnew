...
function Split-RegPath([string]$Path) {
    # \\ -> key/value split when the value name has a \ in it
    $i = $Path.IndexOf('\\')
...
# advanced audit isn't in RSoP WMI -> gpresult
...
# on by default -> old-style audit ignored once any advanced audit is set
...
# manifests usually only use the secpol.msc name -> ask the provider for the real one
...
foreach ($r in $catalog.resources) {
    ...
    # one resource = a whole audit CSV -> check each row
    if ($r.type -eq 'Dsc_auditpolicycsv') {
        ...
        if ($gpoOnly) { $skipped.Add("$ref (restores the whole audit policy -> resets $gpoOnly GPO-only audit subcategories till the next GP refresh)") }
        continue
    }
    $id = $null
    $value = $null
